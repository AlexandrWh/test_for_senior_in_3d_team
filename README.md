...
| `data/cq500_test/align/previews/` | golden PNG до/после (2×3 MPR, ~97 кейсов) |
...
│   ├── align/                            # результат HeadAligner (локально)
│   │   ├── volumes/                      # NIfTI @ 1 mm (локально, не в git)
│   │   ├── meta/                         # JSON по кейсу (локально)
│   │   ├── results.csv                   # локально
│   │   └── previews/                     # в git — PNG до/после
...
**В репозиторий:** JSON из `cq500_train_guides/`, `no_heads.txt`, golden-превью в `align/previews/`, при желании `train_logs/` и `mask_residual/`.
...
Результат: `data/cq500_test/align/volumes/*.nii.gz` @ 1 mm, meta, CSV (локально); PNG before/after — в `align/previews/` (коммитятся в git).
...
docker compose up -d
python -u scripts/run_head_align_golden.py --service-url http://localhost:8000
python -u scripts/render_align_previews.py   # перегенерировать PNG при необходимости
python -u scripts/eval_mask_residual_angles.py   # опционально, нужны masks
```

Качественные результаты без прогона: `data/cq500_test/align/previews/` (уже в репозитории).
...
Превью (в git): `data/cq500_test/align/previews/{case}_before_after.png` — можно смотреть без локальных volumes.
